...
const savedLogo=localStorage.getItem('meraj_logo');if(savedLogo&&document.querySelector('#logoMark img'))document.querySelector('#logoMark img').src=savedLogo;
function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function initHeroSlideshow(){
 const broken=['assets/images/1000029568.jpg','assets/images/1000029579.jpg','assets/images/1000029603.jpg'];
 const slides=get('meraj_gallery',defaultGallery).map(x=>x.src).filter(src=>src&&!broken.includes(src));
...
function render(){let g=get('meraj_gallery',defaultGallery);galleryGrid.innerHTML=g.map(x=>`<article class="gallery-item"><img src="${x.src}" alt="${esc(x.title)}" onerror="this.onerror=null;this.src='assets/images/school-anniversary.jpg'"><div><b>${esc(x.title)}</b><br><small>${esc(x.description||'')}</small></div></article>`).join('');renderMedia()}
...
galleryFilter.onchange=e=>{let all=get('meraj_gallery',defaultGallery),v=e.target.value,g=v==='all'?all:all.filter(x=>x.category===v);galleryGrid.innerHTML=g.map(x=>`<article class="gallery-item"><img src="${x.src}" alt="${esc(x.title)}" onerror="this.onerror=null;this.src='assets/images/school-anniversary.jpg'"><div><b>${esc(x.title)}</b><br><small>${esc(x.description||'')}</small></div></article>`).join('')||'<p>No images in this category yet.</p>'};
admissionForm.onsubmit=e=>{e.preventDefault();const form=e.target;const file=form.studentPassport.files[0];if(!file){formMsg.textContent='Please upload the student passport.';return}const reader=new FileReader();reader.onload=()=>{const data=Object.fromEntries(new FormData(form));delete data.studentPassport;data.studentPassport={name:file.name,type:file.type,dataUrl:reader.result};let a=get('meraj_admissions',[]);a.push({...data,submittedAt:new Date().toISOString()});save('meraj_admissions',a);formMsg.textContent='Application saved in this GitHub demo browser. Real submissions will use the Hostinger backend.';form.reset()};reader.readAsDataURL(file)};render();
